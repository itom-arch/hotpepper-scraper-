...
def get_or_create_sheet(sh, sheet_name, header):
    try:
        ws = sh.worksheet(sheet_name)
    except gspread.WorksheetNotFound:
        ws = sh.add_worksheet(title=sheet_name, rows=10000, cols=60)
        ws.update("A1", [header])
...
def write_to_sheets(results, vacancy_data, results_tadasu, today, sh):
    ...
    safe_write_row(ws_review, [today] + [r["reviews"] for r in results])

    ws_blog = get_or_create_sheet(sh, SHEET_BLOG, [""] + salon_names)
    safe_write_row(ws_blog, [today] + [r["blogs"] for r in results])

    ws_vac = get_or_create_sheet(sh, SHEET_VACANCY, ["", "\u53d6\u5f97\u65e5", "\u5bfe\u8c61\u65e5"] + salon_names)
    ...
    for target_date in all_dates:
        ...
        rows_vac.append(row)
    if rows_vac:
        safe_write_rows(ws_vac, rows_vac)

    ws_review_t = get_or_create_sheet(sh, SHEET_REVIEW_TADASU, [""] + salon_names_tadasu)
    safe_write_row(ws_review_t, [today] + [r["reviews"] for r in results_tadasu])

    ws_blog_t = get_or_create_sheet(sh, SHEET_BLOG_TADASU, [""] + salon_names_tadasu)
    safe_write_row(ws_blog_t, [today] + [r["blogs"] for r in results_tadasu])
    ...
    async with async_playwright() as p:
        ...
        for url in URLS:
            try:
                ...
                print(f"ok {result['name']}: review={result['reviews']} blog={result['blogs']}")
            except Exception as e:
                results.append({"name": "error", "reviews": 0, "blogs": 0})
                print(f"err SILK {url}: {e}")
        ...
        for i, url in enumerate(URLS):
            ...
            try:
                ...
                print(f"ok vacancy {name}: {len(vac)} days")
            except Exception as e:
                vacancy_data[name] = {}
                print(f"err vacancy {name}: {e}")
            await asyncio.sleep(2)
        ...
        for url in URLS_TADASU:
            try:
                ...
                print(f"ok TADASU {result['name']}: review={result['reviews']} blog={result['blogs']}")
            except Exception as e:
                results_tadasu.append({"name": "error", "reviews": 0, "blogs": 0})
                print(f"err TADASU {url}: {e}")
            await asyncio.sleep(2)

        await browser.close()

    write_to_sheets(results, vacancy_data, results_tadasu, today, sh)
    print(f"all done: {today}")
